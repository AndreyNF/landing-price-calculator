...
def create_or_update_user(
    ...
    if existing:
        ...
        cursor.execute(f"""
            INSERT INTO {schema}users (telegram_id, name, avatar_url, email_verified, login, password_hash, created_at, updated_at, last_login_at)
            VALUES (%s, %s, %s, TRUE, %s, '', NOW(), NOW(), NOW())
            RETURNING id, email, name, avatar_url, telegram_id
        """, (telegram_id, display_name, photo_url, tg_login))
