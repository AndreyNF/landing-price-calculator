        ...
        try:
            ...
            if row:
                ...
                if vk_email and row:
                    ...
                    # 3. Create new user
                    vk_login = f"vk_{vk_user_id}"
                    cur.execute(
                        f"""INSERT INTO {S}users
                            (vk_id, email, name, avatar_url, email_verified, login, password_hash, created_at, updated_at, last_login_at)
                            VALUES (%s, %s, %s, %s, TRUE, %s, %s, %s, %s, %s)
                            RETURNING id""",
                        (str(vk_user_id), vk_email, full_name, photo_url, vk_login, '', now, now, now)
                    )
